...
- **URLクエリによるコード読み込み（`exerciseId`/`codeId`）**: BhvVisualizerの公開読み取り専用API（認証不要）からコードを取得しエディタへ反映する、`# BHV:` タグなしの通常機能。`src/core/exercise-source.js`の`loadExerciseFromQuery()`が`app.js`から呼ばれる。クエリが無い場合は何もせず、既定のFibonacciサンプル表示・21種の組み込みサンプルは変更しない（加算的な機能）。動作検証は `verify-exercise-query.mjs`（`node verify-exercise-query.mjs`）。設計判断の背景は [ADR-029](docs/adr/ADR-029-url-query-exercise-loading.md)
