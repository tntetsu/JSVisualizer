...
式・文・関数呼び出しの各粒度でステップ実行しながら、**12 種類の可視化ビュー**でプログラムの動作（時間の経過とともにメモリの内容がどう変化するか）を直感的に理解できます。
...
| | 実行トレース | humanStep の実行順に全ステップを一覧。配列＋ポインタのミニ図（ポインタが検出されたステップのみ、表示枠の幅はドラッグで変更可）・変数値列・条件式列を表示。変化した値を橙太字で強調。while/for の条件式はイテレーションごとに値を表示 |
