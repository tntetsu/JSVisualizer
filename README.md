...
式・文・関数呼び出しの各粒度でステップ実行しながら、多彩な可視化ビューでプログラムの動作（時間の経過とともにメモリの内容がどう変化するか）を直感的に理解できます。
...
タブ切り替えで以下のビューを利用できます（✅ 実装済み、🔧 開発予定）。

| カテゴリ | ビュー | 状態 | 説明 |
|----------|--------|------|------|
| **基本** | 変数・スタック | ✅ | 変数値・コールスタック・コンソール出力の統合パネル |
| | スコープ | ✅ | スコープチェーンをネスト枠で表現 |
| | コールスタック | ✅ | 関数呼び出しのスタックフレーム（アニメーション付き） |
| **トレース** | アニメーション付きトレース表 | ✅ | ステップごとに行が追記されるトレース表 |
| | 全ステップ表 | ✅ | 全ステップを一覧できる静的テーブル |
