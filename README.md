...
| **トレース** | トレース表 | 行番号列にソース先頭 15 文字スニペットを表示する変数マトリクス表。列の表示/非表示・D&D 並び替え可 |
| | 実行トレース | humanStep の実行順に全ステップを一覧。変数値列・条件式列を表示。while/for の条件式はイテレーションごとに値を表示 |
...
| | 配列 | 複数の配列を色付き箱でインデックス付き表示。ポインタ変数は変数ごとに個別行で表示。各配列ブロックは枠線＋背景色で区切り、幅不足時は次行へ折り返し。ステップ間で配列位置が動かないよう最大サイズで領域を確保 |
| | 時系列 | 変数値の推移を SVG 折れ線グラフで。変数チップ選択時に Y 軸スケールを動的再計算。カーソル縦線が同期 |
