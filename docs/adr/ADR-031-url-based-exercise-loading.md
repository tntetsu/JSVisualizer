...
> **2026-08-12追加変更（ADR-033）**: 本ADRでは`exercise`のコード群をサンプル選択に「追加」する設計だったが、実機確認の結果、組み込みサンプルと混在すると見つけにくいことが判明し、`exercise`/`code`指定時は組み込みサンプルを取り除いて指定されたコードだけを表示する方式に変更した。詳細は[ADR-033](ADR-033-hide-builtin-samples-when-remote.md)を参照。
